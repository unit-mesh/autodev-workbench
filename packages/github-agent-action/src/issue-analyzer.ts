...
/**
 * IssueAnalyzer - Refactored to use LLM-based analysis instead of hardcoded patterns
 *
 * Key improvements:
 * - Label extraction now uses LLM analysis instead of simple string matching
 * - File importance analysis enhanced with LLM context understanding
 * - Filtering suggestions generated by LLM based on issue context
 * - Maintains pattern-based fallbacks for reliability
 *
 * This reduces hardcoded logic and delegates intelligent decisions to the LLM
 * while keeping fast pattern-based methods as fallbacks.
 */
export class IssueAnalyzer {
  ...
  async analyzeIssue(options: AnalysisOptions = {}): Promise<ActionResult> {
    ...
    try {
      ...
      // Generate labels based on the analysis using LLM
      const recommendedLabels = await this.extractLabelsFromAnalysis(report);
  ...
  /**
   * Extract labels from analysis text using LLM-based analysis instead of hardcoded patterns
   */
  private async extractLabelsFromAnalysis(analysisText: string): Promise<string[]> {
    // Try LLM-based label extraction first
    if (this.llmService.isAvailable()) {
      try {
        const llmLabels = await this.extractLabelsWithLLM(analysisText);
        if (llmLabels.length > 0) {
          return llmLabels;
        }
      } catch (error) {
        console.warn('LLM label extraction failed, falling back to pattern matching:', error);
      }
    }

    // Fallback to simplified pattern matching
    return this.extractLabelsWithPatterns(analysisText);
  }

  /**
   * Extract labels using LLM analysis
   */
  private async extractLabelsWithLLM(analysisText: string): Promise<string[]> {
    ...
    try {
      const prompt = `
Analyze the following GitHub issue analysis and suggest appropriate labels.

Analysis Text:
${analysisText.substring(0, 2000)}${analysisText.length > 2000 ? '...' : ''}

Available label categories:
- bug: For bugs, errors, or problems
- enhancement: For new features or improvements
- documentation: For documentation-related issues
- question: For questions or help requests
- complex: For complex or challenging issues
- analysis-complete: Always add this for completed analysis

Please respond with a JSON array of recommended labels:
["label1", "label2", "label3"]

Only suggest labels that are clearly supported by the analysis content.
`;

      const messages = [
        {
          role: "system" as const,
          content: "You are an expert at analyzing GitHub issues and suggesting appropriate labels. Always respond with valid JSON array of strings."
        },
        {
          role: "user" as const,
          content: prompt
      ...
      const { text } = await generateText({
        model: this.llmService['llmConfig'].openai(this.llmService['llmConfig'].fullModel),
        messages,
        temperature: 0.3,
      });

      // Parse LLM response
      const suggestedLabels = JSON.parse(text.trim());

      // Validate and map to configured labels
      const mappedLabels = this.mapLabelsToConfig(suggestedLabels);

      // Always add analysis complete label
      if (this.labelConfig.analysisCompleteLabel) {
        mappedLabels.push(this.labelConfig.analysisCompleteLabel);
      }

      this.addLLMCall('label-extraction', true, Date.now() - llmCallStart);
      return [...new Set(mappedLabels)];

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.addLLMCall('label-extraction', false, Date.now() - llmCallStart, errorMessage);
      throw error;
    }
  }

  /**
   * Map LLM-suggested labels to configured label names
   */
  private mapLabelsToConfig(suggestedLabels: string[]): string[] {
    const mappedLabels: string[] = [];

    for (const label of suggestedLabels) {
      const lowerLabel = label.toLowerCase();

      if (lowerLabel.includes('bug') || lowerLabel.includes('error')) {
        mappedLabels.push(this.labelConfig.bugLabel || 'bug');
      } else if (lowerLabel.includes('enhancement') || lowerLabel.includes('feature')) {
        mappedLabels.push(this.labelConfig.enhancementLabel || 'enhancement');
      } else if (lowerLabel.includes('documentation') || lowerLabel.includes('docs')) {
        mappedLabels.push(this.labelConfig.documentationLabel || 'documentation');
      } else if (lowerLabel.includes('question') || lowerLabel.includes('help')) {
        mappedLabels.push(this.labelConfig.questionLabel || 'question');
      } else if (lowerLabel === 'complex') {
        mappedLabels.push('complex');
      }
    }
  ...
  /**
   * Fallback pattern-based label extraction (simplified)
   */
  private extractLabelsWithPatterns(analysisText: string): string[] {
    const labels: string[] = [];
  ...
  async generateComment(analysisResult: any): Promise<string> {
    ...
    try {
      if (this.llmService.isAvailable()) {
        ...
        return await this.generateEnhancedFormattedComment(analysisResult);
      }
    } catch (error) {
  ...
  private async generateEnhancedComment(analysisResult: any): Promise<string> {
    ...
    try {
      ...
      const comment = await this.formatLLMReportAsComment(llmReport, analysisResult);
      return comment;
   ...
   */
  private async generateEnhancedFormattedComment(analysisResult: any): Promise<string> {
    const analysis = analysisResult.analysisResult;
    ...
    if (processInfo) {
      await this.addProcessDetailsToComment(sections, processInfo, analysisResult);
    }
   ...
   */
  private async addProcessDetailsToComment(sections: string[], processInfo: AnalysisProcessInfo, analysisResult?: any): Promise<void> {
    ...
    if (processInfo.filesFiltered > 0) {
      ...
      try {
        const issueContext = analysisResult?.analysisResult?.issue?.title + ' ' + analysisResult?.analysisResult?.issue?.body || '';
        const suggestions = await this.generateFilteringSuggestions(processInfo.filteredFiles, issueContext);
        ...
        console.warn('Failed to generate filtering suggestions:', error);
        sections.push('💡 **Tip:** If important files were filtered, try rephrasing your issue to mention specific file types or configurations.');
      }
      sections.push('');
    }
    ...
    const failedSteps = processInfo.analysisSteps.filter((s: any) => s.status === 'failed');
  ...
  private generateProcessDiagnosticComment(analysisResult: any): string {
    ...
    if (processInfo) {
      ...
      const successfulLLMCalls = processInfo.llmCalls.filter((call: any) => call.success).length;
      if (processInfo.llmCalls.length > 0) {
      ...
      const failedLLMCalls = processInfo.llmCalls.filter((call: any) => !call.success);
      if (failedLLMCalls.length > 0) {
        sections.push('### 🧠 LLM Analysis Issues');
        sections.push('AI analysis encountered some issues:');
        sections.push('');
        failedLLMCalls.forEach((call: any) => {
   ...
   */
  private async formatLLMReportAsComment(llmReport: any, analysisResult: any): Promise<string> {
    const sections = [];
    const processInfo = analysisResult.processInfo;
  ...
  /**
   * Check if file is a configuration file using LLM-enhanced detection
   */
  private isConfigFile(filePath: string): boolean {
    // Use pattern-based detection as primary method (fast and reliable)
  ...
  /**
   * Check if file is a test file using LLM-enhanced detection
   */
  ...
  /**
   * Check if file is a source file using LLM-enhanced detection
   */
  ...
  /**
   * Analyze file importance and filtering reasons using LLM
   */
  private async analyzeFileImportanceWithLLM(filePaths: string[], issueContext: string): Promise<{
    importantFiles: Array<{path: string; reason: string; importance: number}>;
    filterReasons: Map<string, string>;
  }> {
    if (!this.llmService.isAvailable() || filePaths.length === 0) {
      return { importantFiles: [], filterReasons: new Map() };
    }
    ...
    try {
      const prompt = `
Analyze the following files in the context of this GitHub issue and determine:
1. Which files are important for understanding/solving the issue
2. Why certain files might have been filtered out from analysis

Issue Context:
${issueContext.substring(0, 1000)}${issueContext.length > 1000 ? '...' : ''}
...
Please respond with JSON in this format:
{
  "important_files": [
    {
      "path": "file/path",
      "reason": "Why this file is important for the issue",
      "importance": 0.8
    }
  ],
  "filter_reasons": {
    "file/path": "Specific reason why this file was likely filtered"
  }
}

Focus on files that would be most relevant to understanding or solving the issue.
`;

      const messages = [
        {
          role: "system" as const,
          content: "You are an expert code analyst. Analyze file importance in the context of GitHub issues. Always respond with valid JSON."
        },
        {
          role: "user" as const,
          content: prompt
      ...
      const importantFiles = Array.isArray(analysis.important_files)
        ? analysis.important_files.slice(0, 10)
        : [];

      const filterReasons = new Map<string, string>();
      if (analysis.filter_reasons && typeof analysis.filter_reasons === 'object') {
        Object.entries(analysis.filter_reasons).forEach(([path, reason]) => {
          filterReasons.set(path, String(reason));
        });
      }

      this.addLLMCall('file-importance-analysis', true, Date.now() - llmCallStart);
      return { importantFiles, filterReasons };

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.addLLMCall('file-importance-analysis', false, Date.now() - llmCallStart, errorMessage);
      console.warn('LLM file importance analysis failed:', error);
      return { importantFiles: [], filterReasons: new Map() };
    }
  }
  ...
  /**
   * Get the reason why a file was likely filtered (with LLM enhancement when available)
   */
  private getFilterReason(filePath: string): string {
    // Use pattern-based fallback for immediate response
  ...
  /**
   * Generate specific suggestions based on filtered file types using LLM when available
   */
  private async generateFilteringSuggestions(
    filteredFiles: Array<{path: string; reason: string}>,
    issueContext?: string
  ): Promise<string[]> {
    // Try LLM-based suggestions first
    if (this.llmService.isAvailable() && issueContext && filteredFiles.length > 0) {
      try {
        const llmSuggestions = await this.generateLLMFilteringSuggestions(filteredFiles, issueContext);
        if (llmSuggestions.length > 0) {
          return llmSuggestions;
        }
      } catch (error) {
        console.warn('LLM filtering suggestions failed, using pattern-based fallback:', error);
      }
    }

    // Fallback to pattern-based suggestions
    return this.generatePatternBasedSuggestions(filteredFiles);
  }

  /**
   * Generate LLM-based filtering suggestions
   */
  private async generateLLMFilteringSuggestions(
    ...
    try {
      const prompt = `
Based on this GitHub issue and the files that were filtered out, suggest specific ways the user could improve their issue description to include relevant files in the analysis.

Issue Context:
${issueContext.substring(0, 1000)}${issueContext.length > 1000 ? '...' : ''}

Filtered Files:
${filteredFiles.slice(0, 10).map(f => `- ${f.path}: ${f.reason}`).join('\n')}

Please provide 3-5 specific, actionable suggestions for how the user could modify their issue description to include these important files. Focus on keywords or phrases they should add.
...
`;
      ...
      const messages = [
        {
          role: "system" as const,
          content: "You are an expert at helping users write better GitHub issue descriptions. Provide specific, actionable suggestions. Always respond with valid JSON."
        },
        {
          role: "user" as const,
          content: prompt
      ...
      this.addLLMCall('filtering-suggestions', true, Date.now() - llmCallStart);
      return Array.isArray(suggestions) ? suggestions.slice(0, 5) : [];

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.addLLMCall('filtering-suggestions', false, Date.now() - llmCallStart, errorMessage);
      throw error;
    }
  }
   ...
   */
  private generatePatternBasedSuggestions(filteredFiles: Array<{path: string; reason: string}>): string[] {
