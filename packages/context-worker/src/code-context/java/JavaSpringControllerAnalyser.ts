...
/**
 * An implementation of RestApiAnalyser for analyzing Java Spring Controllers
 */
@injectable()
export class JavaSpringControllerAnalyser extends RestApiAnalyser {
  readonly langId: LanguageIdentifier = 'java';
  private config: LanguageProfile;
  private parser: Parser | undefined;
  private language: Parser.Language | undefined;
  
  // Custom query for Spring annotations
  ...
  analyse(node: CodeStructure, workspacePath: string): void {
    if (!this.language || !this.parser) {
      console.warn('JavaSpringControllerAnalyser not initialized');
      return;
    }
