...
//         '/test/person.rs',
//         rustCode,
//         rustProfile.symbolExtractor.getQuery()
//       );
//
//       // 验证基本的符号提取
//       expect(symbols.length).toBeGreaterThan(0);
...
//         '/test/person.go',
//         goCode,
//         goProfile.symbolExtractor.getQuery()
//       );
//
//       // 验证基本的符号提取
//       expect(symbols.length).toBeGreaterThan(0);
...
//         '/test/person.c',
//         cCode,
//         cProfile.symbolExtractor.getQuery()
//       );
//
//       // 验证基本的符号提取
//       expect(symbols.length).toBeGreaterThan(0);
...
//       expect(functionSymbols.length).toBeGreaterThanOrEqual(6); // create_person, get_name, get_age, greet, free_person, main
//
//       // 验证主函数符号
//       const mainFunction = symbols.find(s => s.kind === SymbolKind.Function && s.name === 'main');
//       expect(mainFunction).toBeDefined();
...
//
//       // 验证函数和注释的关联
//       const createPersonFunc = symbols.find(s => s.kind === SymbolKind.Function && s.name === 'create_person');
//       expect(createPersonFunc).toBeDefined();
//       if (createPersonFunc) {
//         const commentText = cCode.substring(createPersonFunc.commentRange.start, createPersonFunc.commentRange.end);
//         expect(commentText).toContain('Creates a new person');
//       }
//     });
//   });
//
