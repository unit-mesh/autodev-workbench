...
| - 文件编辑        | `str-replace-editor`    | `Edit`             | `edit_file`        | `insert-edit-into-file` | `edit_file`            | `str-replace-editor` | `edit_file_fast_apply`, `update_file` |
| - 文件创建        | `write-file`            | ❌ 无                | `write_to_file`    | `create-file`           | `edit_file` (创建模式)     | `save-file`          | `create_file`                         |
...
| - 安全策略        | 用户确认机制                  | 内置检测机制             | 用户确认机制             | 命令白名单                   | 用户确认机制                 | 用户确认机制               | 命令白名单                                 |
...
| - 文件模式搜索      | `search-keywords`       | `GlobTool`         | `grep_search`      | `file-search`           | `search_symbol`        | `view` (搜索模式)        | `search_by_keyword`                   |
| - 内容搜索        | `code-search-regex`     | `GrepTool`         | `grep_search`      | `grep-search`           | `search_symbol`        | `view` (正则搜索)        | `search_by_regex`                     |
| - 语义搜索        | ❌ 无                     | ❌ 无                | `codebase_search`  | ❌ 无                     | `search_codebase`      | `codebase-retrieval` | ❌ 无                                   |
| - 符号搜索        | ❌ 无                     | ❌ 无                | `view_code_item`   | ❌ 无                     | `search_symbol`        | ❌ 无                  | `get_definition`, `get_reference`     |
...
| **网络功能**      | ✅ 完整套件                  | ❌ 无                | ✅ 基础支持             | ✅ 完整套件                  | ✅ 完整套件                 | ✅ 完整套件               | ⚠️ 有限支持                               |
| - 网页抓取        | `web-fetch-content`     | ❌ 无                | `read_resource`    | `web-fetch`             | `fetch_content`        | `web-fetch`          | ❌ 无                                   |
| - 网络搜索        | `web-search`            | ❌ 无                | ❌ 无                | `web-search`            | `search_web`           | `web-search`         | ❌ 无                                   |
...
### Trae AI 核心优势
1. **全面的文件操作能力** - 支持文件的读、写、创建、删除、重命名和编辑。
2. **灵活的代码搜索与导航** - 提供关键字、正则表达式搜索，以及符号定义和引用查找。
...
6.  **持续学习与适应**：从交互中学习，不断优化策略和对工具的运用。
    ...
    | **扩展性** | 插件化架构 | 工具固定 | 插件化架构 | 插件化架构 | 工具固定 | 插件化架构 | 工具集固定 |
