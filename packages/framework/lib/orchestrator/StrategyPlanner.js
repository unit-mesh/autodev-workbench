...
class StrategyPlanner extends ContextAwareComponent_1.ContextAwareComponent {
    ...
    assessComplexity(analysis) {
        ...
        if (analysis.files.length > 100)
            score += 2;
        else if (analysis.files.length > 50)
            score += 1;
        // 依赖数量
        ...
        if (depCount > 50)
            ...
            score += 1;
        // 特殊文件
        if (analysis.files.some(f => f.includes('webpack')))
            score += 1;
        if (analysis.files.some(f => f.includes('babel')))
            score += 1;
        if (score >= 4)
            return 'high';
        if (score >= 2)
    ...
    determineMigrationType(analysis) {
    ...
    determineTargetVersion(analysis) {
        const targetMap = {
    ...
    generateStepConfig(step, analysis) {
        const configs = {
            'dependency-upgrade': {
    ...
    estimateDuration(analysis, template) {
...
exports.StrategyPlanner = StrategyPlanner;
//# sourceMappingURL=StrategyPlanner.js.map